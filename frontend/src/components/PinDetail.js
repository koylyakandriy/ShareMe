		...
		if (query) {
			client.fetch(query)
				.then(([ currentPin ]) => {
					...
					if (currentPin) {
						query = pinDetailMorePinQuery(currentPin);
			...
			<div className="flex xl-flex-row flex-col m-auto bg-white"
				...
				<div className="w-full p-5 flex-1 xl:min-w-620">
					...
					<Link to={ `/user-profile/${ pinDetail.postedBy?._id }` }
						...
						<p
							className="font-semibold capitalize">{ pinDetail.postedBy?.userName }</p>
					</Link>
					<h2 className="mt-5 text-2xl">Comments</h2>
					<div className="ma-h-370 overflow-y-auto">
					...
					<div className="flex flex-wrap mt-6 gap-3 items-center">
						...
						<input type="text"
						       className="flex-1 border-gray-100 outline-none border-2 p-2 rounded-2xl focus:border-gray"
